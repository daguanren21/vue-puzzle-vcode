  ...
  <div class="min-h-screen bg-slate-100 text-slate-800">
    <div class="mx-auto max-w-3xl px-6 py-12">
      ...
      <p class="mt-2 text-sm text-slate-500">
        Vue 2.7 playground · Radix-style composable parts + default composition
      </p>
    ...
    <!-- 1. 默认组合组件:与 v1 用法一致(Vue 2 用 .sync) -->
    <Vcode
      :show.sync="show1"
      :imgs="imgs"
      @success="(d) => { push(`默认组件 验证成功 diff=${d.toFixed(1)}`); show1 = false }"
      @fail="(d) => push(`默认组件 验证失败 diff=${d.toFixed(1)}`)"
      @close="push('默认组件 关闭')"
    />
    ...
    <VcodeRoot
      :show.sync="show2"
      @success="(d) => { push(`组合式 验证成功 diff=${d.toFixed(1)}`); show2 = false }"
      @fail="(d) => push(`组合式 验证失败 diff=${d.toFixed(1)}`)"
      @close="push('组合式 关闭')"
    >
      <VcodePortal>
